...
num_samples = 12500  # 1000 samples as default
...
text_model.add(tf.keras.layers.Embedding(vocab_size, word_vector_dimension,
                                         ...
                                         mask_zero=True))
text_model.add(tf.keras.layers.LSTM(100, return_sequences=True))
text_model.add(tf.keras.layers.LSTM(20, return_sequences=False))
...
num_epoch = 50  # 50
my_batch_size = 10  # 5
text_model.fit(X_train_padded, y_train, epochs=num_epoch, verbose=True, batch_size=my_batch_size)
