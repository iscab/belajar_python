...
class Residual(tf.keras.layers.Layer):
    ...
    def call(self, x):
        ...
        fx = tf.keras.layers.Conv2D(self.channels_in, self.kernel, activation="relu", padding="same")(x)
        fx1 = tf.keras.layers.BatchNormalization()(fx)
        fx2 = tf.keras.layers.Conv2D(self.channels_in, self.kernel,  padding="same")(fx1)

        # skip connection
        out = tf.keras.layers.Add()([x, fx2])
        out1 = tf.keras.layers.ReLU()(out)
...
# reconstruction_model.add(Residual(32, (3,3)))
reconstruction_model.add(tf.keras.layers.Conv2D(3, (5,5), padding="same", activation="relu"))
...
history = reconstruction_model.fit(zip(train_data_low, train_data_high), batch_size=batch_size, epochs=30)
...
# end of file, version: 12:00 03.01.2024
